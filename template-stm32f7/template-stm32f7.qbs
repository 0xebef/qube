...
import qbs
import "qbs/stm32f7.qbs" as stm32f7
...
Project {
     ...
     * - STM32F779xx
     */
    readonly property string microcontroller_series: "STM32F722xx"
    ...
    readonly property bool   fpu_dp     : true  // use double precision fpu
    readonly property bool   cm4_instr  : false // use Cortex-M4 intruction set
    ...
    stm32f7 {
        ...
        property string halPath       : basePath + "/Drivers/STM32F7xx_HAL_Driver"
        ...
        cpp.includePaths: {
            ...
            if (project.cmsis) {
                ...
                includePaths.push(cmsisPath + "/Device/ST/STM32F7xx/Include")
            }
            ...
            if (project.freertos) {
                ...
                if (project.cm4_instr) {
                    includePaths.push(freertosPath + "/Source/portable/GCC/ARM_CM4F")
                } else {
                    includePaths.push(freertosPath + "/Source/portable/GCC/ARM_CM7/r0p1")
                }
            }
        ...
        cpp.staticLibraries: {
            ...
            if (project.cmsis_math) {
                if (project.cm4_instr) {
                    ...
                    if (project.fpu) {
                        if (project.fpu_dp) {
                            staticLibraries.push("arm_cortexM7lfdp_math")
                        } else {
                            staticLibraries.push("arm_cortexM7lfsp_math")
                        }
                    } else {
                        staticLibraries.push("arm_cortexM7l_math")
                    }
                }
            }
            ...
            if (project.audio) {
                if (project.cm4_instr) {
                    ...
                    if (project.fpu) {
                        staticLibraries.push("PDMFilter_CM7F_GCC")
                    } else {
                        staticLibraries.push("PDMFilter_CM7_GCC")
                    }
                }
            }
            ...
            if (project.stemwin) {
                if (project.cm4_instr) {
                    ...
                    if (project.freertos) {
                        if (qbs.buildVariant === "release") {
                            staticLibraries.push("STemWin" + project.stemwin_v + "_CM7_OS_GCC_ot")
                        } else {
                            staticLibraries.push("STemWin" + project.stemwin_v + "_CM7_OS_GCC")
                        }
                    } else {
                        if (qbs.buildVariant === "release") {
                            staticLibraries.push("STemWin" + project.stemwin_v + "_CM7_GCC_ot")
                        } else {
                            staticLibraries.push("STemWin" + project.stemwin_v + "_CM7_GCC")
                        }
                    }
                }
            }
